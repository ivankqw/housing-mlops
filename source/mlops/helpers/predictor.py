...
class Predictor:
    ...
    def data_quality_checks(self, input_data: pd.DataFrame) -> None:
        ...
        check_column(input_data, 'year', lambda x: (x < 0).any() or (
            lambda x: isinstance((int, float))), "Year must be a non-negative integer.")
        #  if (input_data['year'] < 0).any() or ~(input_data['year'].apply(lambda x: isinstance(x, (int,float)))).all():

        # Check for invalid values in 'month'
        check_column(input_data, 'month', lambda x: (x.between(1, 12) & (
            x.astype(int) == x)).all(), "Month must be an integer between 1 and 12.")

        # Check for invalid values in 'remaining_lease_years'
        check_column(input_data, 'remaining_lease_years', lambda x: (x >= 0).all() and (
            x.astype(int) == x).all(), "Remaining lease must be a non-negative integer.")

        # Check for invalid values in 'floor_area_sqm'
        check_column(input_data, 'floor_area_sqm', lambda x: (
            x >= 0).all(), "Floor area must be a non-negative float or integer.")

        # Check for invalid values in 'flat_type'
        valid_flat_types = ['1 ROOM', '2 ROOM', '3 ROOM',
                            '4 ROOM', '5 ROOM', 'EXECUTIVE', 'MULTI-GENERATION']
        check_column(input_data, 'flat_type', lambda x: x.isin(
            valid_flat_types).all(), "Invalid flat type.")

        # Check for valid data types in 'flat_model', 'storey_range', and 'district'
        check_column(input_data, 'flat_model', lambda x: x.apply(
            lambda y: isinstance(y, str)).all(), "Flat model must be a string.")
        check_column(input_data, 'storey_range', lambda x: x.apply(
            lambda y: isinstance(y, str)).all(), "Storey range must be a string.")
        check_column(input_data, 'district', lambda x: x.apply(
            lambda y: isinstance(y, (int, float))).all(), "District must be a string.")
    ...
    def preprocess_input(self, input_data: pd.DataFrame) -> pd.DataFrame:
        ...
        input_data['flat_type_ordinal'] = input_data['flat_type'].replace(
            flat_type_mapping)
        ...
        train_set = pd.read_csv(self.data_path)
